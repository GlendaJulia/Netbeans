     ...
     */
      ...
      HttpServletResponse response) throws ServletException, IOException {
        ...
        try {
            ...
            String xtip = request.getParameter("xtip");
            String xcod = "";
            cBaseDatos objDB = new cBaseDatos();
            if (objDB.validarUsuario(xnom, xcla, xtip )){
                HttpSession misession= request.getSession(true);
                misession.setAttribute("usuario",xnom.toUpperCase());
                
                xcod = objDB.codigo(xnom);
                misession.setAttribute("id_alumno",xcod);
                if (xtip.equals("Profesor")){
                    response.sendRedirect("/LAb04/areas.jsp");
                }else if (xtip.equals("Estudiante")) {
                    response.sendRedirect("/LAb04/alumno.jsp");
                }
            }else
                response.sendRedirect("/LAb04/index.html");
        } finally { 
            out.close();
        }
